...
def run_an_eval_epoch(smiles_list,args, model, data_loader):
    ...
    with torch.no_grad():
        ...
        out=pd.read_csv("T-data_predictions(chembl32_logD).csv")
        out['predict']=round(df['standard_value'],3)
        out.to_csv('T-data_predictions(chembl32_logD)_copy.csv', index=False)
...
if __name__ == '__main__':
    ...
    args = setup(args)
    test_set = pd.read_csv('T-data_predictions(chembl32_logD).csv')
